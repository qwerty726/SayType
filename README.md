...
- **本地历史记录**：每次识别自动追加到 `~/.voice_input/history.jsonl`（含时间戳、原文、润色后、所用引擎），可在设置中关闭
...
每次识别成功后会在 `~/.voice_input/history.jsonl` 追加一条记录（语音指令不计入）：

```json
{"ts": "2026-05-25T14:32:10.123+08:00", "original": "你好世界", "polished": null, "backend": "funasr_streaming"}
```

相关配置项（写入 `config.json` 即可调整，UI 开关在后续版本提供）：

| key | 默认值 | 说明 |
|---|---|---|
| `history_enabled` | `true` | 是否记录历史；关闭后立即停止写入 |
| `history_max` | `500` | 最多保留的条数；超出后按时间裁剪只留最近 N 条 |
| `history_record_polish` | `true` | LLM 润色开启时是否同时记录润色后文本 |
